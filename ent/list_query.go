...
func (lq *ListQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*List, error) {
	...
	if query := lq.withTasks; query != nil {
		if err := lq.loadTasks(ctx, query, nodes,
			func(n *List) { n.Edges.Tasks = []*Task{} },
			func(n *List, e *Task) { n.Edges.Tasks = append(n.Edges.Tasks, e) }); err != nil {
			return nil, err
		}
	}
	if query := lq.withUsers; query != nil {
		if err := lq.loadUsers(ctx, query, nodes,
			func(n *List) { n.Edges.Users = []*User{} },
			func(n *List, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *ListQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*List, init func(*List), assign func(*List, *Task)) error {
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(list.TasksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.list_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "list_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "list_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (lq *ListQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*List, init func(*List), assign func(*List, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*List)
	nids := make(map[uuid.UUID]map[*List]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(list.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(list.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(list.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(list.UsersPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*List]struct{}{byID[outValue]: {}}
