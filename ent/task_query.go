...
func (tq *TaskQuery) QueryList() *ListQuery {
	query := (&ListClient{config: tq.config}).Query()
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withList; query != nil {
		if err := tq.loadList(ctx, query, nodes, nil,
			func(n *Task, e *List) { n.Edges.List = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TaskQuery) loadList(ctx context.Context, query *ListQuery, nodes []*Task, init func(*Task), assign func(*Task, *List)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		if nodes[i].list_tasks == nil {
			continue
		}
		fk := *nodes[i].list_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "list_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Task, init func(*Task), assign func(*Task, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(task.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.UsersPrimaryKey[0]))
